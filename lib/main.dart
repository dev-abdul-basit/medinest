      ...
      if (notificationResponse.payload!.contains('notificationMid')) {
        ...
        if (notificationResponse.actionId == takenId) {
          ...
          await flutterLocalNotificationsPlugin.cancel(
            notificationId + Constant.notificationStartID,
          );
          ...
          DateTime notificationTime = NotificationHelper()
              .getDateTimeFromTZDateTime(scheduledDate);
          notificationTable.nNotificationTime = notificationTime
              .toIso8601String();
...
Future<void> takeMedicine({
  ...
  required MedicineNotificationTable notificationTable,
}) async {
  ...
  var result = await DataBaseHelper.instance.insertHistoryData(
    historyTableData,
  );
  ...
  if (await InternetConnectivity.isInternetConnect()) {
    await FireStoreHelper().addAndUpdateHistory(
      historyTableData.hId!,
      historyTableData,
    );
  }
...
Future<void> acceptAppointment({
  ...
  var result = await DataBaseHelper.instance.insertAppointmentHistoryData(
  ...
  if (await InternetConnectivity.isInternetConnect()) {
    await FireStoreHelper().addAndUpdateAppointmentHistory(
      appointmentHistoryTableData,
    );
  }

  await flutterLocalNotificationsPlugin.cancel(
    appointmentNotificationTable.anId!,
  );
}
      ...
      builder: (context, orientation, deviceType) {
        ...
        SystemChrome.setSystemUIOverlayStyle(
          SystemUiOverlayStyle(
            ...
            statusBarIconBrightness: Utils.isLightTheme()
                ? Brightness.dark
                : Brightness.light,
